<script lang="ts">
	...
	import { ExternalLink, Send, AlertCircle, CheckCircle } from 'lucide-svelte';
	...
	function handleSubmit() {
		...
		if (!isValidUrl(url)) {
			formError = 'Please enter a valid URL (must start with http:// or https://)';
			return;
		}
...
<div class="container mx-auto px-4 py-8 max-w-2xl">
	<div class="mb-8">
		...
		<p class="text-gray-600 dark:text-gray-400">
			Share interesting content by providing a URL. Our system will automatically scrape and process the content.
		</p>
	</div>
			...
			use:enhance={() => {
				...
				return async ({ result, update }) => {
					...
					if (result.type === 'success') {
						formMessage = 'Content submitted successfully! We\'ll process it shortly.';
						resetForm();
					} else if (result.type === 'failure') {
						formError = (result.data as any)?.message || 'Failed to submit content. Please try again.';
					}
					await update();
				};
			}}
			...
			<div class="space-y-6">
				...
				<div class="space-y-2">
					...
					<p class="text-xs text-gray-500 dark:text-gray-400">
						Enter the URL of the article, blog post, or other content you'd like to submit.
					</p>
				</div>
				...
				<div class="space-y-2">
					...
					></textarea>
					<p class="text-xs text-gray-500 dark:text-gray-400">
						Optional: Add context or explain why this content is interesting.
					</p>
				</div>

				<!-- Error Message -->
				{#if formError}
					<div class="flex items-center space-x-2 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md">
						<AlertCircle size={16} class="text-red-600 dark:text-red-400 flex-shrink-0" />
				...
				<!-- Success Message -->
				{#if formMessage}
					<div class="flex items-center space-x-2 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md">
						<CheckCircle size={16} class="text-green-600 dark:text-green-400 flex-shrink-0" />
						<p class="text-sm text-green-700 dark:text-green-300">{formMessage}</p>
					</div>
				{/if}
	...
	<!-- Help Section -->
	<div class="mt-8 p-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg shadow-sm">
		...
		<ul class="space-y-2 text-sm text-blue-800 dark:text-blue-200">
			...
			<li class="flex items-start space-x-2">
				...
				<span>The content will be processed and included in the upcoming podcast episode</span>
			</li>
		</ul>
	</div>
</div>
