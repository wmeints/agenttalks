...
public class BuzzsproutActivitiesImpl implements BuzzsproutActivities {
    ...
    @Override
    public String publishPodcastEpisode(String title, String description, String showNotes, String audioFileUrl) {
        logger.infof("Publishing podcast episode to Buzzsprout: %s", title);

        try {
            CreateEpisodeRequest request = new CreateEpisodeRequest(title, description, showNotes, audioFileUrl);
